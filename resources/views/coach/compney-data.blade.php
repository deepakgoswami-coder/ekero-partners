...
<body class="flex items-center justify-center min-h-screen p-4">
 ...
 function registerNow(id) {
        ...
        // 1. Path Parameter Placeholder के साथ route URL generate करें।
        // Note: Laravel `route()` helper जब array लेता है, तो Path parameter को Path में और बाकी keys को Query parameters में बदलता है। 
        // लेकिन यहाँ हम सिर्फ Placeholder का उपयोग करेंगे।
        // हम एक TEMP_ID Placeholder का उपयोग करेंगे ताकि हम JavaScript में इसे सही ढंग से बदल सकें।
        // यदि आप Laravel 8+ का उपयोग कर रहे हैं, तो URL में placeholder `:id` के बजाय TEMP_ID का उपयोग करना safer है।
        
        let baseUrl = "{{ route('user.register', ['id' => 'TEMP_ID']) }}"; 
        
        // 2. Placeholder 'TEMP_ID' को वास्तविक 'id' से बदलें।
        let url = baseUrl.replace('TEMP_ID', id);
        
        // 3. Query Parameter (?shares=X) जोड़ें।
        // Note: यदि baseUrl में पहले से कोई query parameter नहीं है, तो हमें '?' का उपयोग करना चाहिए।
        // चूंकि 'TEMP_ID' एक Path Parameter है, इसलिए URL में '?' अभी तक नहीं आया होगा।
        url = url + '?shares=' + encodeURIComponent(shareCount);
