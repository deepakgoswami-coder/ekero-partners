                ...
                <div class="content-header-right text-md-end col-md-6 col-12 d-md-block d-none">
                    <div class="mb-1 breadcrumb-right">
                        <div class="dropdown">
                            ...
                            <a href="{{ route('users.astro.create') }}" class=" btn btn-primary ">Add Astrologer</a>
                                        ...
                                        <tbody>
                                            @foreach ($users as $key => $user)
                                                <tr>
                                                    ...
                                                    <td>{{ $user->name ?? '' }}</td>
                                                    <td>{{ $user->email ?? '' }}</td>
                                                    <td>{{ $user->role == 2 ? 'User' : "astrologer" }}</td>
                                                    <td>{{ $user->phone_number ?? '' }}</td>
                                                    <td>{{ $user->created_at ?? '' }}</td>
                                                    <td>
                                                        <div class="d-flex flex-column">
                                                            <div class="form-check form-switch form-check-primary">
                                                                <input type="checkbox" onchange="toggleButton({{ $user->id }})"
                                                    ...
                                                    <td>
                                                        <div>
                                                            <a href="{{ route('users.edit',$user->id) }}"><img width="20px" src="{{ asset('admin/icons/edit.png') }}" alt=""></a>
...
@section('script')
    ...
    <script>
        ...
        function toggleButton(id) {
            $.ajax({
                url: "{{ route('web.toggle_user_status') }}",
                type: "POST",
                data: {
