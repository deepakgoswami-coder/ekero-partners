  ...
  public function dashboard()
{
    ...
    public function myContribution()
    {
        ...
        if ($groupId) {
            $group = Group::find($groupId); // find() direct ID leta hai
            $groupMembers = GroupMember::where('group_id', $groupId)->get();
            ...
            $userMember = GroupMember::where('user_id', $user->id)
                                    ->where('group_id', $groupId)
                                    ->first();
            $weeklyCommitment = $userMember ? $userMember->weekly_commitment : 0;
        }

        $contributions = Contribution::where('user_id', $user->id)->latest()->get();

        return view('user.contribution', compact(
            'user', "group", "portal", "leader", 
            "groupMembers", "contributions", 'weeklyCommitment'
        ));
    }
